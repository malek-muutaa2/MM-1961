...
export function ForecastUploader({onUploadSuccess, isSubmitted = false}: ForecastUploaderProps) {
    ...
    if (isSubmitted) {
        return (
            <div className="space-y-4">
                <div
                    className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-12 text-center dark:border-gray-700 dark:bg-gray-900">
    ...
    return (
        <div className="space-y-6">
            {ability.can('create', 'Forecast') ? (
                <div>
                    {!uploading && !validating && !uploadComplete && validationIssues.length === 0 && (
                        <div
                            className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center dark:border-gray-700">
                ...
                <div className="flex items-center justify-between rounded-lg border p-4">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">You do not have permission to upload forecasts.</p>
                        </div>
                    </div>
                </div>
            )}
                                ...
                                <div key={index} className="px-4 py-3">
                                    <div className="flex items-start space-x-3">
                                        ...
                                        <div>
                                            ...
                                            {issue.column &&
                                                <p className="text-sm text-muted-foreground">Column: {issue.column}</p>}
                                            {issue.row !== undefined &&
                                                <p className="text-sm text-muted-foreground">Row: {issue.row}</p>}
                                        </div>
                                    </div>
                                </div>
                            ))}
            ...
            {uploadComplete && (
                <div className="space-y-4">
                    <Alert>
                        ...
                        <AlertDescription>
                            Your forecast file has been uploaded, validated, and submitted successfully. No further
                            uploads are
