...
type KPIID = "available" | "partial" | "unavailable"

export function KPISelectionForm({
                                     category,
                                     onKPIsSelected,
                                     selectedKPIs: externalSelectedKPIs = [],
                                 }: Readonly<KPISelectionFormProps>) {
    ...
    const getDataAvailability = (kpiId: string): KPIID => {
        switch (kpiId) {
            case "expiry-writeoffs":
            case "inventory-accuracy":
            case "inventory-carrying-cost":
    ...
    //         ? "warning"
    //         : "secondary"
    const getVariant = (dataAvailability: "available" | "partial" | "unavailable") => {
    ...
    const getBadgeClass = (dataAvailability: "available" | "partial" | "unavailable") => {
        switch (dataAvailability) {
            case "available":
        ...
        <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
                {kpis.map((kpi) => {
                    ...
                    return (
                        <Card key={kpi.id}
                              className={`border ${internalSelectedKPIs.includes(kpi.id) ? "border-primary" : ""}`}>
                            <CardHeader className="pb-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <Badge
                                            variant={getVariant(dataAvailability)}
                                            className={getBadgeClass(dataAvailability)}
                                        >
                                            {dataAvailability === "available" ? (
